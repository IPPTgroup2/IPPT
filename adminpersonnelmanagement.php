...
<head>
  ...
  <link rel="stylesheet" href="styles/adminpersonnelmanagement.css">
</head>
  ...
  <form method="GET" action="">
    ...
    <button type="submit"><i class="fa fa-search"></i></button>
      ...
      <div class="personnel-btn">
          <a href="adminaddpersonnel.php"><i class='fas fa-user-plus'></i> Add Personnel</a>
      </div>

<div class="table-container">
<form method="post" action="adminpersonnelmanagement.php">
    <table>
      <tbody> 
      <?php
 // Initialize variable for search name
if (isset($_GET['search'])) {
  $searchName = $_GET['search'];
} 

 // Modify the attendance query to include the search condition
 $attendanceSql = "SELECT fname, mname, lname, facultynumber FROM personnel WHERE fname LIKE '%$searchName%' OR mname LIKE '%$searchName%' OR lname LIKE '%$searchName%'";
$attendanceResult = $con->query($attendanceSql);

if ($attendanceResult->num_rows > 0) {
    while ($nameRow = $attendanceResult->fetch_assoc()) {
        $fname = $nameRow['fname'];
        $mname = $nameRow['mname'];
        $lname = $nameRow['lname'];
        ...
        $name = "$fname $mname $lname";
        echo "<tr>";
        echo "<td><a href=\"adminpersonnelinformation.php?name=" . urlencode($name) . "&facultynumber=" . urlencode($facultynumber) . "&fname=" . urlencode($fname) . "&lname=" . urlencode($lname) . "\">$name</a></td>";
        echo "<td><div class=\"button-container\"><a href=\"admineditpersonnelinfo.php?facultynumber=" . urlencode($facultynumber) . "&fname=" . urlencode($fname) . "&lname=" . urlencode($lname) . "\"><i class='fas fa-edit'></i></a></div></td>";
        echo "<td><div class=\"button-container\"><a href=\"adminremovepersonnel.php?facultynumber=" . urlencode($facultynumber) . "&fname=" . urlencode($fname) . "&lname=" . urlencode($lname) .  "\"><i class='fas fa-trash-alt'></i></a></div></td>";
        echo "</tr>";
    }
} else {
    echo "<tr><td colspan=\"3\" style=\"text-align: center;\">No personnel found.</td></tr>";
}
?>
