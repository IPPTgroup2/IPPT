...
<head>
  ...
  <link rel="stylesheet" href="styles/adminpersonnelinformation.css">
</head>
...
<div class="table-container">
    ...
    if (isset($_GET['fname']) && isset($_GET['lname']) && isset($_GET['facultynumber'])) {
        ...
        // Retrieve the personnel's information from the database using the full name and facultynumber
        $personnelSql = "SELECT * FROM personnel WHERE fname='$fname' AND lname='$lname' AND facultynumber='$facultynumber'";
        $personnelResult = $con->query($personnelSql);

        if ($personnelResult->num_rows > 0) {
            $personnelRow = $personnelResult->fetch_assoc();

            echo '<table class="personnel-table">';
            echo '<tr>';
            echo '<th>Field</th>';
            echo '<th>Value</th>';
            echo '</tr>';

            // Loop through the personnel information fields and display them in rows
            foreach ($personnelRow as $field => $value) {
                echo '<tr>';
            ...
            echo "<td><div class=\"edit\"><a href=\"admineditpersonnelinfo.php?facultynumber=" . urlencode ($facultynumber). "&fname=" . urlencode($fname) . "&lname=" . urlencode($lname) . "\"><i class='fas fa-edit'></i></a></div></td>";
            echo "<td><div class=\"remove\"><a href=\"adminremovepersonnel.php?facultynumber=" . urlencode ($facultynumber). "&fname=" . urlencode($fname) . "&lname=" . urlencode($lname) . "\"><i class='fas fa-trash-alt'></i></a></div></td>";
            echo "</tr>";
            echo '</div>';
          } else {
          ...
          echo "<p>Invalid request. Personnel name not provided.</p>";
        }
        ?>
</div>
  </section>
